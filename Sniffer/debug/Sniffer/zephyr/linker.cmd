...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0x10000000 + 0x10000), LENGTH = (0x6c000 - 0x4000)
    RAM (wx) : ORIGIN = 0x30000000, LENGTH = (104 * 1K)
    RetainedMem ( rw ) : ORIGIN = (805412864), LENGTH = (8192) rpmsg_sh_mem ( rw ) : ORIGIN = (1486653440), LENGTH = (6144)
 ...
 *(.iplt)
 ...
 __rom_region_start = (0x10000000 + 0x10000);
    rom_start :
 {
...
/DISCARD/ :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    RetainedMem (NOLOAD) : { __RetainedMem_start = .; KEEP(*(RetainedMem)) KEEP(*(RetainedMem.*)) __RetainedMem_end = .; } > RetainedMem __RetainedMem_size = __RetainedMem_end - __RetainedMem_start; __RetainedMem_load_start = LOADADDR(RetainedMem); rpmsg_sh_mem (NOLOAD) : { __rpmsg_sh_mem_start = .; KEEP(*(rpmsg_sh_mem)) KEEP(*(rpmsg_sh_mem.*)) __rpmsg_sh_mem_end = .; } > rpmsg_sh_mem __rpmsg_sh_mem_size = __rpmsg_sh_mem_end - __rpmsg_sh_mem_start; __rpmsg_sh_mem_load_start = LOADADDR(rpmsg_sh_mem);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
...
noinit (NOLOAD) :
{
    ...
    __kernel_ram_end = 0x30000000 + (104 * 1K);
